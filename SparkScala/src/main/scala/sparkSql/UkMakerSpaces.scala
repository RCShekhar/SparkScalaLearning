...
object UkMakerSpaces {
  ...
  def main(args: Array[String]) {
    ...
    val makerSpace = session.read.option("header", "true").csv("src/main/scala/in/uk-makerspaces-identifiable-data.csv")

    val postCode = session.read.option("header", "true").csv("src/main/scala/in/uk-postcode.csv")
    ...
    joined.groupBy("Region").count().sort("count").show()
  }
}
